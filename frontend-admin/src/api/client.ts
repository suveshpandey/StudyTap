...
// Password is generated by backend, but schema requires it (we send empty string)
export type UniversityAdminCreateResponse = {
  id: number;
  university_id: number;
  is_active: boolean;
...
): Promise<UniversityAdminCreateResponse> => {
  const res = await apiClient.post<UniversityAdminCreateResponse>(
    `/master/universities/${universityId}/create-admin`,
    data // Backend generates password automatically
  );
  return res.data;
};
...
export const masterGetUniversityBranches = async (universityId: number): Promise<Branch[]> => {
  const res = await apiClient.get<Branch[]>(`/master/universities/${universityId}/branches`);
  return res.data;
};
